...
class DALIOutputs(object):
    def __init__(self, out_dict):
        ...
        assert 'encoder_outputs' in out_dict and 'importance_maps' in out_dict
        ...
        if self._has_processed_signal:
            self._outs = (
                ...
                out_dict['logits'],
                out_dict['maps'],
                out_dict['encoder_outputs'],
                out_dict['importance_maps'],
            )
        else:
            self._outs = (
                out_dict['audio'],
...
class _AudioTextDALIDataset(Iterator):
    ...
    def __next__(self):
        ...
        # TODO: check
        # Loading teacher_encoder_outputs
        file = f"{DIR}/teacher_encoder_outputs.pkl"
        file_obj = open(file, "rb") # read binary
        teacher_encoder_outputs = pickle.load(file_obj) # Tensor
        ...
        out['encoder_outputs'] = teacher_encoder_outputs
        out['importance_maps'] = teacher_importance_map
